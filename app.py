...
from models import Users, db, Admins  # Ensure your `models.py` defines Users and initializes db
...
app.config["UPLOAD_FOLDER"] = (
    "static/product_images"  # Define upload folder for product images
)
...
def index():
    ...
    if not is_database_connection_successful(DB_URL):
        return redirect(
            url_for("install")
        )  # Redirect to install page if DB is not connected
...
def install():
    if request.method == "POST":
        ...
        if not all(
            [host, port, user, password, dbname, username, email, admin_password]
        ):
        ...
        if not is_database_connection_successful(new_db_url):
            flash(
                "Не удалось подключиться к базе данных. Проверьте данные и попробуйте снова.",
                "danger",
            )
            return redirect(url_for("install"))
