...
application.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///cwim_db.db'
application.config['SECRET_KEY'] = "kljdklsahiopduy1y298e319hdskajh"
application.config['UPLOAD_FOLDER'] = 'static/product_images'  # Define upload folder for product images
...
@application.route('/install', methods=['GET', 'POST'])
def install():
...
def index_page():
    if request.method == "POST":
        ...
        for image in product_images:
            if image:
                image_filename = os.path.join(application.config['UPLOAD_FOLDER'], image.filename)
...
if __name__ == '__main__':
    application.run(host=WEBHOOK_LISTEN, debug=False)
